...
export async function populateReference(doc: any, key: string) {
  ...
  let orderSpec = __documents[docTypeName]["ordered"]
    ? __documents[docTypeName]["ordered"][key]
    : null;
...
async function unwind(
  targetType,
  value,
  resolveValue: (targetType, v: any) => Promise<any>
) {
  if (!value) {
    return value;
  }
...
export async function fromDB<Type extends Collection>(
  type: Function,
  doc: any,
  dontDeserialize?: boolean,
  resolveLazyReferences?: boolean | Array<string>
): Promise<Type> {
  if (!doc) {
    return doc;
  }
  ...
  for (var i = 0; i < keys.length; i++) {
    ...
    if (
      ...
      resolveLazyReferences === true ||
      (isArray(resolveLazyReferences) &&
        ~(<Array<string>>resolveLazyReferences).indexOf(key))
    ) {
...
export function setupDocument(type: Function) {
  ...
  if ((<any>type).find) {
    (<any>type).find = (<any>type).find.bind(
      _.extend(type, {
        __type: type,
      })
    );
